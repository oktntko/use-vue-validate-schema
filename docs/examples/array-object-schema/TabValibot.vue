...
const ItemSchema = v.object({
  ...
  item: v.pipe(v.string(), v.minLength(1), v.maxLength(50)),
  cost: v.pipe(v.number(), v.integer(), v.minValue(-10_000), v.maxValue(10_000)),
  quantity: v.pipe(v.number(), v.integer(), v.gtValue(0), v.maxValue(100)),
  price: v.pipe(v.number(), v.integer(), v.minValue(-10_000 * 100), v.maxValue(10_000 * 100 * 100)),
});
const schema = v.object({
  issue_date: v.pipe(v.string(), v.isoDate()),
  due_date: v.pipe(v.string(), v.isoDate()),
  invoice_to: v.pipe(v.string(), v.minLength(1), v.maxLength(10)),
  details: v.pipe(v.array(ItemSchema), v.minLength(1)),
});
...
function newItem() {
  return {
    ...
    quantity: 0,
    price: 0,
  } satisfies v.InferInput<typeof ItemSchema>;
}
</script>
