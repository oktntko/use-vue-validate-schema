...
const schema = v.object({
  text: v.pipe(v.string(), v.trim(), v.minLength(1), v.maxLength(10)),
  textarea: v.pipe(v.string(), v.maxLength(100)),
  single_checkbox: v.pipe(v.boolean()),
  multiple_checkbox: v.pipe(v.array(v.picklist(multiple_checkbox_options)), v.minLength(1)),
  radio: v.picklist(radio_options),
  single_select: v.pipe(
    v.pipe(
      v.string(),
      v.check((val) => val !== '', 'Please select one'),
    ),
    v.picklist(single_select_options),
  ),
  multiple_select: v.pipe(v.array(v.picklist(multiple_select_options)), v.minLength(1)),
});
